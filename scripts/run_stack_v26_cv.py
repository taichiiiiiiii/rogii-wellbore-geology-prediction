"""Stack v2.6: 56 original features + H3 ONLY (59 cols) x 5-seed (issue: R13).

**Why R13.** R12 (``run_stack_v25_cv.py``) established that of the three
transfer-safe raw-feature groups, only **H3** carries signal: the cumulative
ablation gave (a) 56-feat repro 9.1839 / (b) +H1 9.1945 (worse) / (c) +H1+H2
9.1835 (flat) / (d) +H1+H2+H3 **9.1466** (-0.0377 vs R11's 9.1843; the c->d
step was -0.0369, and ``known_linear_trend_extrap_minus_anchor`` ranked #2 in
config d's importances). H1 alone HURT (+0.0102) and H2 was flat, so R13 asks:
does dropping the dead-weight H1/H2 groups and keeping only H3 (56 + 3 = 59
features) match or beat R12(d)'s 9.1466? Expected: same to slightly better.

**Design.** Identical harness to R11/R12 (same fold seed 42, same ES split
seed 42, same LGBM base params, same 5 seeds ``(42, 101, 202, 303, 404)``,
one OS process per seed, strictly serial). The feature matrix is stack v2's
56 columns plus :func:`build_h3_features`'s 3 columns (byte-copied from
``run_stack_v25_cv.py``) -- H1/H2 are **not computed at all** (which also
drops v25's per-well H1 lag search, so the matrix build is faster).

**Gate.** 5-seed mean pooled OOF <= **9.0843** (R11 9.1843 - 0.10, same
adoption gate as R12), reported regardless of outcome. The R13-specific
comparison -- printed alongside -- is vs **R12(d) = 9.1466** ("what did
dropping H1+H2 buy/cost").

**Leak boundary.** Strict subset of R12's: H3 reads only ``MD``, the
known-zone (``TVT_input`` non-NaN prefix) ``TVT_input``, and ``Z``. No
``h["TVT"]`` reads, no eval-zone ``TVT_input``, no train-only formation
columns, no pf_bma_v2/beam_grid bank npz.

Usage::

    # smoke test (all 5 seeds + aggregate in one process, ~30 wells)
    uv run python scripts/run_stack_v26_cv.py --n-wells 30

    # full 773-well run, ONE PROCESS PER SEED, strictly serial
    uv run python scripts/run_stack_v26_cv.py --seed-idx 0
    ...
    uv run python scripts/run_stack_v26_cv.py --seed-idx 4

    # after all 5 members have saved outputs/stack_v26_result_s*.npz:
    uv run python scripts/run_stack_v26_cv.py --aggregate
"""
...
R12_CONFIG_D_POOLED_RMSE = 9.1466  # ledger 2026-07-10 (run_stack_v25_cv.py config d 5-seed mean)
PF_BLEND_W = 0.7
...
def build_well_matrix_v26(wells: list[str], split: str = "train") -> _WellMatrix:
    ...
    H1/H2 are never computed (R13 drops them entirely).
    """
    counts = {
    ...
    if not used_wells:
        raise ValueError(
            f"build_well_matrix_v26: 0 wells passed validation -- no rows to build "
            f"(counts={counts})"
        )
    ...
    if len(all_cols) != N_FEATURES_EXPECTED:
        raise AssertionError(
            f"R13 must train on exactly {N_FEATURES_EXPECTED} (56 + H3) features, "
            f"got {len(all_cols)}"
        )
    return _WellMatrix(
...
def run_one_seed(
    ...
    label = f"seed {seed}"
    print(f"\n=== {label} (R13 56+H3 config, n_features={len(cols)}) ===", flush=True)
...
def _result_path(seed: int, n_wells: int | None) -> Path:
    ...
    return OUTPUTS_DIR / f"stack_v26_result_s{seed}{suffix}.npz"
...
def _load_result(seed: int, n_wells: int | None) -> dict[str, object]:
    ...
    if not path.exists():
        raise FileNotFoundError(
            f"{path} not found -- run `uv run python scripts/run_stack_v26_cv.py "
...
def _aggregate(n_wells: int | None) -> None:
    ...
    print()
    print("=" * 96)
    print(f"R13 (56 + H3 = {len(ref['cols'])} features)")
    print(
        f"{'k':>3}{'seed':>7}{'solo pooled':>14}{'cum-mean pooled (1..k)':>24}"
        f"{'vs R11 9.1843':>16}{'vs R12d 9.1466':>16}"
    )
    print("-" * 96)
    ...
    for k, res in enumerate(results, start=1):
        ...
        print(
            ...
            f"{cum_pooled - R12_CONFIG_D_POOLED_RMSE:>+16.4f}"
        )
    print("=" * 96)
    ...
    print(
        f"\n{len(results)}-seed mean OOF (R13): pooled={mean_pooled:.4f}  "
    ...
    if seed42 is not None:
        _helped_hurt_report(
            f"R13 {len(results)}-seed mean",
            y_true,
            oof_mean,
            seed42["oof"],
            f"R13 seed-42 solo ({seed42['pooled_rmse']:.4f})",
            well_idx,
            n_wells_used,
        )

    out_path = OUTPUTS_DIR / "stack_v26_oof.npz"
    save_kwargs = {
    ...
    delta_r12d = mean_pooled - R12_CONFIG_D_POOLED_RMSE
    verdict = (
        "採用候補" if mean_pooled <= ADOPTION_GATE_RMSE else "却下（非改善、実測値は報告のみ）"
    )
    print(f"\n{'=' * 96}")
    print(
        f"判定 (R13): {verdict} -- 5-seed mean={mean_pooled:.4f}, "
        f"R11 baseline={R11_5SEED_MEAN_POOLED_RMSE:.4f} (改善 {improvement_r11:+.4f}ft), "
        f"R12(d)={R12_CONFIG_D_POOLED_RMSE:.4f} (差 {delta_r12d:+.4f}ft), "
...
def main() -> None:
    ...
    parser.add_argument(
        "--aggregate",
        action="store_true",
        help="Skip CV; load outputs/stack_v26_result_s*.npz, print the table, "
        "write outputs/stack_v26_oof.npz.",
    )
    ...
    t0 = time.time()
    wm = build_well_matrix_v26(wells, split="train")
    gc.collect()
    print(
        f"feature matrix (R13, 56+H3 = {wm.X.shape[1]} cols): {wm.X.shape}, "
