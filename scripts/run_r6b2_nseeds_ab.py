"""R6-b(2) A/B: does doubling the PF-BMA seed count improve pf_bma_scale3?

Hypothesis under test (analysis/experiment_ledger.md, 2026-07-10 R6-b dissection,
improvement queue item (2)): the adopted PF-BMA runs ``n_seeds = 12``
independent particle-filter realizations per well (a dev-box time-gate
choice, scripts/run_pf_bma_cv.py Phase 0); the public reference notebook
``sunnywu27/rogii-wellbore-tvt-physical-model`` uses 128 seeds. More seeds
give the seed-softmax (BMA over cumulative log-likelihood) more independent
realizations to choose from, which should matter most on wells where
individual seeds mode-collapse onto wrong branches -- but with diminishing
returns, and a real cost: on the Kaggle bank-builder kernel, doubling seeds
adds roughly +50 min per ~200 wells.

This script measures OFF (n_seeds=12, the adopted config) vs ON (n_seeds=24)
pooled RMSE for ``pf_bma_scale3`` with **both arms at the new production
setting** ``init_pos_std=2.0`` (R6-b(3) adopted, honest CV 9.0502;
fill_gr_gaps=False on both arms -- R6-b(1) was measured and rejected), on
...
``pf_bma_scale3_tvt`` column (the init_pos_std=2.0 bank v2;
implementation-regression guard).

Adoption threshold is ``delta <= -0.15`` (looser than R6-b(3)'s -0.30
because a seed doubling needs no new mechanism, but not free -- see the
kernel-time cost above).

Usage::

    nohup uv run python scripts/run_r6b2_nseeds_ab.py \
        > outputs/r6b2_nseeds_ab.log 2>&1 &
"""
...
N_WELLS_SAMPLE = 150
N_PARTICLES = 512
N_SEEDS_OFF = 12  # adopted config (must reproduce bank v2)
N_SEEDS_ON = 24  # doubled
BMA_SCALES: tuple[float, ...] = (3.0, 5.0, 8.0, 12.0)
GR_SCALES: tuple[float, ...] = (8.0, 12.0, 20.0, 30.0)
TARGET_SCALE = 3.0
INIT_POS_STD = 2.0  # new production setting (R6-b(3) adopted), BOTH arms
ADOPTION_DELTA = -0.15  # looser than R6-b(3)'s -0.30; kernel cost ~+50min/200wells
...
def load_bank_scale3(used_wells_wanted: list[str]) -> dict[str, np.ndarray]:
    ...
    with np.load(BANK_PATH, allow_pickle=True) as npz:
        ...
        assert int(npz["n_seeds"]) == N_SEEDS_OFF, (
            f"bank v2 n_seeds {int(npz['n_seeds'])} != OFF arm {N_SEEDS_OFF}"
        )
...
def main() -> None:
    ...
    print(
        f"\n# config: n_particles={N_PARTICLES} bma_scales={BMA_SCALES} "
        f"gr_scales={GR_SCALES} target_scale={TARGET_SCALE} "
        f"init_pos_std={INIT_POS_STD} (both arms) fill_gr_gaps=False (both arms)"
    )
    print(f"# OFF: n_seeds={N_SEEDS_OFF}   ON: n_seeds={N_SEEDS_ON}")
    ...
    print(f"  OFF (n_seeds={N_SEEDS_OFF}, existing behaviour): {pooled_off:.4f}")
    print(f"  ON  (n_seeds={N_SEEDS_ON}):                      {pooled_on:.4f}")
    ...
    print(
        ...
        "=> proceed to bank rebuild (seed doubling costs ~+50min/200wells on the kernel)"
    )
