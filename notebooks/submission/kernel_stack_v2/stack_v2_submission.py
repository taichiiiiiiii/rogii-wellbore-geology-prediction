"""Self-contained Kaggle Notebook submission: stack v2 all-in (train-in-kernel).
...
- locally against ``data/raw/`` for validation::
    ...
    uv run python notebooks/submission/kernel_stack_v2/stack_v2_submission.py
    ROGII_SMOKE=20 uv run python notebooks/submission/kernel_stack_v2/stack_v2_submission.py

Method: train-in-kernel
------------------------
Unlike ``notebooks/submission/kernel_pf`` (which ports a fixed, untrained
blend formula), this kernel **trains a LightGBM model from scratch inside the
Kaggle rerun**: it (1) runs the PF + beam trackers and the spatial-prior
predictor over every train well AND every test well, (2) builds the stack v2
"all-in" feature matrix (P1 within-well features + tracker features +
well-aggregate tracker features + spatial-prior features) for the train
wells, (3) fits one LightGBM model on that matrix with a well-level
early-stopping holdout, and (4) predicts every test well's evaluation zone
with the trained model, falling back to ``carry_last`` per-well on any
failure. This reproduces ``scripts/run_stack_v2_cv.py``'s "all-in" ablation
row (current local best: pooled OOF RMSE **9.2309**, honest ES-holdout
protocol -- see ``analysis/experiment_ledger.md``, 2026-07-07 "stack v2"
row), minus the OOF/CV bookkeeping (there is nothing to score on Kaggle's
held-out test set; the ES-holdout split here plays the same *early-stopping*
role the CV script's fold-internal ES-holdout played, just carved from *all*
773 train wells instead of one fold's training wells).

Runtime budget (<=9h Kaggle CPU)
----------------------------------
The dominant cost is Phase 2: PF + beam + spatial-prior computation over
every train AND test well (feature assembly, the ES-holdout split, LightGBM
training, and test prediction are all comparatively cheap -- see the smoke
measurement below, where Phase 2 was 185.6s out of a 187.7s total run).

Two data points, both honest (neither rounded up nor cherry-picked):

1. **Component ledger figures** (this repo's dev machine,
   ``analysis/experiment_ledger.md`` 2026-07-06 rows, PF/beam only, no
   spatial): PF **3.11s/well** (8 seeds x 512 particles) + beam
   **1.44s/well** (3-config DP ensemble) = ~4.6s/well.
2. **This file's own local smoke-test measurement**
   (``ROGII_SMOKE=20``, 20 train + 3 test = 23 wells, PF + beam + spatial +
   the per-row nearest-neighbor-distance diagnostic combined): **8.071s/well**
   (185.6s / 23 wells). Higher than (1) because it includes the spatial-prior
   predictor's KD-tree queries and diagnostic, which (1) does not cover.

Using the more conservative, more complete measurement (2) as the basis:
``8.071 * 973 (773 train + ~200 hidden test) ~= 7,853s ~= 131 min``. The
spatial-prior bank build itself is a one-time cost, separately measured at
0.4s for a 20-well bank; a full 773-well bank is a low tens-of-seconds cost
(``scripts/build_spatial_cache.py``'s ledger note) and per-well KD-tree query
cost grows only logarithmically with bank size, so 131 min is not expected to
grow much when moving from the 20-well smoke bank to the full 773-well bank.
LightGBM training (one model on ~657 fit wells + ~116 ES-holdout wells, ~56
features) is a minutes-scale cost by comparison: the smoke run's 75,510-row
fit took 1.3s (best_iteration=3, an artificially early stop on such a small
dataset); the full run has ~35x more fit rows and will need more boosting
rounds, but even the full 5-config x 5-fold CV experiment in
``run_stack_v2_cv.py`` (25 model fits total) took only 755.7s, so one
full-data model here is very unlikely to exceed single-digit minutes. Total
estimate: **~135-145 min** end to end locally. Even at Kaggle's measured
~1.5x slower-than-local execution (``docs/playbooks/03_kaggle_ops.md``),
that is **~200-220 min (~3.5h)**, leaving several hours of headroom inside
the 9h budget. Actual per-well timing is printed every ``PROGRESS_EVERY``
wells during the run so the budget can be monitored live.
...
residual; the final TVT prediction is ``pf_blend + residual_pred``. Ported
verbatim from (attribution kept per-module below):
...
def output_path(root: Path) -> Path:
    ...
    try:
        ...
        repo_root = Path.cwd()
    return repo_root / "outputs" / "submission_stack_v2.csv"
...
def build_predictions(
    ...
    for i, (well, well_rows) in enumerate(wells):
        try:
            ...
            preds = predict_well_stack(h, tw, well, wts_cache, feature_cols, model)
...
def main() -> None:
    ...
    # test well (the dominant cost -- see the time-estimate comment at the
    # top of this file). ---
    ...
    # --- Phase 6: predict every test well (well-level try/except -> carry_last) ---
