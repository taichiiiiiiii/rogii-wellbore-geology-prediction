...
ROGII_DUP2: ID-match contacts-reconstruction fallback (new; v2, strictly
additive on top of v1's ``dup_overlay_submission.py``)
---------------------------------------------------------------------------
This file is a byte-for-byte copy of
``notebooks/submission/kernel_dup_overlay/dup_overlay_submission.py`` (v1,
the ``ROGII_DUP`` overlay described above) with ONE further overlay bolted
on top, never editing or removing any of the logic above: v1's
:func:`find_duplicate_twin` only fires when a hidden test well is a
byte-identical duplicate of a train well (GR match ratio >= 99.9% within
0.01). A re-logged copy of the same physical well (same well ID, but a
fresh log run with small GR sensor noise/drift) would NOT pass that gate
and falls through to the ordinary sub-v8 model prediction, leaving real
train-label information on the table even though the well's identity is
still knowable: the test well's own filename-derived ID is, by construction
of this competition's train/test split (see this repo's memory --
``project_rogii_kaggle.md`` -- and the v1 docstring above; verified on all 3
locally-visible test wells), sometimes literally a train well's ID too.

- For every test well v1's :func:`find_duplicate_twin` did NOT match, this
  overlay checks whether the test well's own ID is present in the train
  well population (:data:`FORMATION_COLS`/:func:`tvt_from_contacts` and
  :func:`_contacts_guard_and_reconstruct` below). If so, it loads that exact
  train well's horizontal-log + typewell (``split="train"``) and, ONLY if a
  strict guard passes, reconstructs the eval-zone ``TVT`` from the train
  copy's own formation-contact depth columns
  (``ANCC/ASTNU/ASTNL/EGFDU/EGFDL/BUDA``, never read from the test well
  itself -- these columns do not exist in the test schema) instead of
  falling back to the sub-v8 model for that well.
- Guard (:func:`_contacts_guard_and_reconstruct`, all conditions required,
  never raises -- returns ``None`` on any failure or gate miss): (1) the
  test well's and the identically-ID'd train well's known-zone
  ``TVT_input`` prefixes agree to within 0.01 ft over >=200 finite rows
  after MD-offset alignment (the same leak-safe check used by v1's own
  :func:`find_duplicate_twin` gate 1 -- ``TVT_input`` is present in the test
  schema, so this is not a leak, only a same-well confirmation); (2) the
  train copy has a formation-depth column (``EGFDU`` preferred, else the
  first :data:`FORMATION_COLS` entry present) with at least one finite value
  over the train rows that correspond, via that same MD offset, to the test
  well's evaluation-zone rows.
- :func:`tvt_from_contacts` (ported from
  ``AmgedAlfaqih/7-091-public (Kaggle)7-091-public.ipynb``'s
  contacts-reconstruction approach, the core mechanism behind that public
  LB-7.091 kernel) inverts the ANCC-style formation-top depth identity
  (``formation_depth ~= TVT + Z`` up to a per-formation reference ``TVT``
  and a per-well additive offset fitted so the reconstruction matches the
  train well's own ``TVT`` column on average) using ONLY the train copy's
  own ``TVT``/``Z``/formation-depth columns and its typewell's own
  ``Geology``/``TVT`` columns -- never anything from the test well besides
  the leak-safe ``MD``/``TVT_input`` alignment/guard check above.
- Toggle: reuses v1's own ``ROGII_DUP`` env var (default ``"1"``/on); there
  is no separate v2 toggle. ``ROGII_DUP=0`` disables BOTH overlays, in which
  case this file's behaviour and ``submission.csv`` output are byte-
  identical to running v1's ``dup_overlay_submission.py`` (and therefore to
  plain sub-v8) under the same conditions.
- This overlay only ever fills in ``dup_results`` entries for test wells v1
  left unmatched -- it never overwrites an entry v1's byte-identical search
  already produced (that direct-copy match is exact, RMSE 0 for that well,
  and always strictly preferred). The final per-row merge
  (:func:`merge_duplicate_overrides`, unchanged from v1) applies to the
  combined ``dup_results`` exactly as it already does for v1's own entries
  -- only finite override rows are touched, everything else keeps the
  unmodified sub-v8 prediction. Any failure at any stage (well load, MD
  offset resolution, guard check, reconstruction) degrades that single well
  back to whatever v1 would have produced for it (i.e. the unmodified sub-v8
  prediction), never aborting the run.

ROGII_DUP3: loose-ID direct TVT-copy fallback (new; v3, strictly additive on
top of v2's ``dup_overlay_v2_submission.py`` -- adds ONLY a third fallback
path, never edits/removes anything above)
---------------------------------------------------------------------------
This file is a byte-for-byte copy of
``notebooks/submission/kernel_dup_overlay_v2/dup_overlay_v2_submission.py``
(v2, the ``ROGII_DUP``/``ROGII_DUP2`` overlay described above, LB 9.014 --
identical to sub-v8 because neither overlay fired on the hidden public
test set) with ONE further overlay bolted on top: v2's
:func:`_contacts_guard_and_reconstruct` guard requires the test well's and
the identically-ID'd train well's known-zone ``TVT_input`` prefixes to
agree to within **0.01 ft** (:data:`_DUP_TVT_INPUT_TOL`) over the MD-aligned
overlap. That tolerance is appropriate for confirming a byte-identical
re-serialization of the same log (v1's own use of it), but it is almost
certainly too strict for a genuinely RE-LOGGED copy of the same physical
well: same well ID, but a fresh acquisition run whose ``GR`` carries sensor
noise/drift and whose ``TVT_input`` differs by a small near-constant
shift/noise from re-processing -- exactly the kind of well v1's byte-match
and v2's 0.01 ft guard would both reject, even though the well's identity is
still knowable from its own ID (the same structural fact v2 already relies
on: this competition's train/test split reuses well IDs, sometimes as
byte-identical dupes, sometimes as re-logged copies).

- For every test well that neither v1's :func:`find_duplicate_twin` NOR v2's
  :func:`_contacts_guard_and_reconstruct` matched, this overlay tries
  :func:`_loose_id_direct_copy`: it re-checks that ``wid`` (the test well's
  own ID) is present among the train wells, resolves the same MD row offset
  as v1/v2 (:func:`_compute_md_offset`), and then applies a DELIBERATELY
  LOOSER known-zone ``TVT_input`` agreement guard -- **prefix RMSE over
  >=100 rows both finite (:data:`_DUP3_MIN_PREFIX_ROWS`) must be < 2.0 ft
  (:data:`_DUP3_PREFIX_RMSE_TOL`)** -- roughly 200x looser than v2's 0.01 ft
  max-abs-diff gate, but still an extremely strong same-well confirmation:
  100+ independent known-zone rows agreeing to within 2 ft RMSE by chance
  for two DIFFERENT wells is not physically plausible given this dataset's
  own measured statistics (the scoreboard section above records a per-well
  constant-TVT oracle pooled RMSE of ~9 ft across ALL wells; two unrelated
  wells' TVT_input tracks would essentially never land within 2 ft RMSE of
  each other over a 100+ row window), while a genuine re-log of the SAME
  physical well -- whose TVT_input differs only by small sensor/processing
  noise and a near-constant re-processing shift -- comfortably passes.
- When the guard passes, :func:`_loose_id_direct_copy` copies the train
  copy's own ``TVT`` column DIRECTLY (not a contacts reconstruction, unlike
  v2) into the test well's evaluation-zone rows, MD-aligned via the same
  integer-offset construction as v1/v2 -- only rows where the train copy's
  ``TVT`` is finite and in range are overridden; unmatched rows keep
  whatever v1/v2/sub-v8 already produced.
- Toggle: reuses v1/v2's own ``ROGII_DUP`` env var (default ``"1"``/on);
  there is no separate v3 toggle. ``ROGII_DUP=0`` disables ALL THREE
  overlays, in which case this file's behaviour and ``submission.csv``
  output are byte-identical to running v2's
  ``dup_overlay_v2_submission.py`` (and therefore to plain sub-v8) under the
  same conditions.
- This overlay only ever fills in ``dup_results`` entries still unmatched
  after v1 (direct byte-match) and v2 (strict contacts reconstruction) --
  it never overwrites an entry either of those already produced (an exact
  byte-identical copy or a strict 0.01 ft-verified contacts reconstruction
  is always strictly preferred over this looser fallback). The final
  per-row merge (:func:`merge_duplicate_overrides`, unchanged from v1/v2)
  applies to the combined ``dup_results`` exactly as it already does for
  v1/v2's own entries -- only finite override rows are touched, everything
  else keeps the unmodified sub-v8 prediction. Any failure at any stage
  (well load, MD offset resolution, guard check) degrades that single well
  back to whatever v1/v2 would have produced for it (i.e. the unmodified
  sub-v8 prediction, if neither of those fired either), never aborting the
  run.

Leak safety (v3 addendum): ``wid`` being present among the train wells is a
structural fact of this competition's data split (test wells sometimes
reuse a train well's ID, including re-logged copies), not information
derived from the test well's own evaluation zone. The guard reads only
``MD``/``TVT_input`` (both present in the test schema, disjoint from the
eval zone by construction: ``known = TVT_input.notna()``,
``eval_m = TVT_input.isna()``) from the test well. The value copied into
the override is the TRAIN copy's own ``TVT`` column -- the train well's
ground truth, already permitted for use against ANY well under this repo's
leak rules ("other train wells' data is fair game for predicting a test
well"; see this file's ``TVT`` leak-boundary notes above for v1's identical
argument about copying a train well's ``TVT`` into a matched test well). If
the loosened guard does not pass, :func:`_loose_id_direct_copy` returns
``None`` and zero rows are overridden for that well -- sub-v8's own
prediction (or v1/v2's, if either fired) stands unchanged, so this fallback
is a strict upper bound on risk: it can only ever replace an already-
produced prediction with a same-well-verified train well's own TVT, never
degrade below whatever v1/v2/sub-v8 would have produced.
"""
...
# --------------------------------------------------------------------------- #
# ROGII_DUP2: ID-match contacts-reconstruction fallback (new; self-contained,
# purely additive on top of the ROGII_DUP block above -- see this file's
# module docstring's "ROGII_DUP2" section for the full rationale/leak-safety
# argument). Reuses v1's own _DUP_MIN_TVT_INPUT_ROWS/_DUP_TVT_INPUT_TOL
# constants and eval_mask/load_horizontal/load_typewell helpers -- nothing in
# this block duplicates a constant already defined above.
# --------------------------------------------------------------------------- #

# Formation-depth columns present only in TRAIN horizontal-well logs (never
# in the test schema). Identical list/order to :data:`FORMATIONS` (the
# spatial-prior predictor's own formation list, defined above) -- aliased
# under the name used by the public-notebook contacts-reconstruction formula
# this block ports, rather than redefined.
FORMATION_COLS: tuple[str, ...] = FORMATIONS


def tvt_from_contacts(
    hw_tr: pd.DataFrame, tw_tr: pd.DataFrame, ref_col: str = "EGFDU"
) -> np.ndarray | None:
    """Reconstruct a train well's own TVT from its formation-contact depths.

    Ported from ``AmgedAlfaqih/7-091-public (Kaggle)7-091-public.ipynb``'s
    contacts-reconstruction approach: the typewell's own ``Geology``/``TVT``
    rows locate a reference formation top's typewell ``TVT`` (``ref_tvt``,
    for ``ref_col``; falls back to the typewell's first logged formation if
    ``ref_col`` is absent from the typewell's ``Geology`` column), and the
    formation-top-depth-vs-TVT identity (``hw_tr[ref_col]`` behaves like
    ``ANCC`` -- see :func:`build_h3_features`'s
    ``known_tvt_z_identity_resid_std`` for the same identity used
    elsewhere in this file) is inverted, with a per-well additive
    ``offset`` fitted so the reconstruction's mean matches the train well's
    own ``TVT`` column exactly (zero mean bias by construction).

    Only ever called with ``hw_tr``/``tw_tr`` loaded from the **train**
    split (this file's ``load_horizontal``/``load_typewell`` with
    ``split="train"``) by :func:`_contacts_guard_and_reconstruct` below --
    ``hw_tr['TVT']`` here is a train well's own ground-truth column, never a
    test well's (the test schema has no ``TVT`` column to read, and this
    function is never handed a test-well DataFrame).

    Never raises: returns ``None`` on any failure (missing column, empty/
    degenerate typewell, non-finite reference TVT).
    """
    try:
        if "Geology" not in tw_tr.columns or "TVT" not in tw_tr.columns:
            return None
        if not {"TVT", "Z", ref_col}.issubset(hw_tr.columns):
            return None
        ...
        if pd.isna(ref_tvt):
            ...
            if ref_col not in hw_tr.columns:
                return None
            ref_tvt = tw_g.loc[tw_g["Geology"] == ref_col, "TVT"].min()
        if pd.isna(ref_tvt) or not np.isfinite(float(ref_tvt)):
            return None
        ...
        reconstructed = ref_tvt - (hw_tr["Z"] - hw_tr[ref_col]) + offset
        return reconstructed.to_numpy(dtype=float)
    except Exception:
        return None


def _compute_md_offset(md_test: np.ndarray, md_train: np.ndarray) -> int | None:
    """Integer row offset aligning ``md_test`` onto ``md_train`` (both 1ft-step grids).

    Same construction as :func:`find_duplicate_twin`'s own offset resolution
    above (``offset = md_first_test - md_first_train``, rounded to the
    nearest integer, rejected if not phase-aligned to the shared 1ft MD
    grid). Returns ``None`` on any degenerate input.
    """
    if md_test.size == 0 or md_train.size == 0:
        return None
    offset_f = float(md_test[0]) - float(md_train[0])
    offset = int(round(offset_f))
    if abs(offset_f - offset) > 1e-6:
        return None
    return offset
...
def _select_formation_guard_col(
    hw_tr: pd.DataFrame, train_rows_valid: np.ndarray
) -> tuple[str | None, float]:
    """Pick the formation-depth column usable for the contacts guard.

    ``EGFDU`` is preferred (matching :func:`tvt_from_contacts`'s own
    default); if absent from ``hw_tr`` or entirely non-finite over
    ``train_rows_valid`` (the train rows that correspond, via the MD
    offset, to the test well's evaluation-zone rows), falls back to the
    first :data:`FORMATION_COLS` entry that IS present with at least one
    finite value there. Returns ``(None, 0.0)`` if none qualifies -- the
    caller then skips the contacts-reconstruction attempt for this well
    entirely (never raises).
    """
    if train_rows_valid.size == 0:
        return None, 0.0
    ordered = ("EGFDU", *(c for c in FORMATION_COLS if c != "EGFDU"))
    for col in ordered:
        if col not in hw_tr.columns:
            continue
        vals = hw_tr[col].to_numpy(dtype=float)[train_rows_valid]
        finite_frac = float(np.mean(np.isfinite(vals))) if vals.size else 0.0
        if finite_frac > 0.0:
            return col, finite_frac
    return None, 0.0


def _contacts_guard_and_reconstruct(
    h_test: pd.DataFrame, wid: str, root: Path
) -> dict[str, object] | None:
    """ID-match contacts-reconstruction fallback for one test well; never raises.

    Only meant to be called when v1's :func:`find_duplicate_twin` found no
    byte-identical twin for this test well AND ``wid`` (the test well's own
    ID) is present among the train wells. Loads the train copy of the SAME
    well ID directly (no search over candidates -- the ID match already
    identifies the well) and, if ALL of the following hold, reconstructs the
    eval zone's ``TVT`` via :func:`tvt_from_contacts`:

      1. The test/train MD grids resolve to an integer row offset
         (:func:`_compute_md_offset`) and agree exactly (``atol=1e-6``) over
         their overlap.
      2. Known-zone ``TVT_input`` agreement over that overlap: >=200 rows
         both finite (:data:`_DUP_MIN_TVT_INPUT_ROWS`, reused from v1), max
         abs diff <=0.01 ft (:data:`_DUP_TVT_INPUT_TOL`, reused from v1) --
         the same leak-safe (``TVT_input`` is present in the test schema)
         same-well confirmation v1's own :func:`find_duplicate_twin` gate 1
         uses.
      3. The train copy has a usable formation-depth column
         (:func:`_select_formation_guard_col`) over the train rows
         corresponding to this test well's evaluation zone.
    ...
    Returns (on success)::

        {
            "twin_well": wid,
            "override": np.ndarray,  # aligned to eval_mask(h_test) row order;
                                     # finite where the reconstruction covers
                                     # that row, NaN elsewhere (leave to caller)
            "diagnostics": {...},
        }

    Returns ``None`` if any gate fails or on ANY internal failure (never
    raises -- the caller always falls back to whatever v1's own overlay/
    sub-v8 pipeline already produced for this well).
    """
    try:
        md_test = h_test["MD"].to_numpy(dtype=float)
        n_test = int(md_test.size)
        ...
        hw_tr = load_horizontal(wid, "train", root)
        tw_tr = load_typewell(wid, "train", root)
        md_train = hw_tr["MD"].to_numpy(dtype=float)
        n_train = int(md_train.size)
        if n_train == 0:
            return None

        offset = _compute_md_offset(md_test, md_train)
        if offset is None:
            return None

        i_lo = max(0, -offset)
        i_hi = min(n_test, n_train - offset)
        if i_hi <= i_lo:
            return None
        test_sl = slice(i_lo, i_hi)
        train_sl = slice(i_lo + offset, i_hi + offset)
        if not np.allclose(md_test[test_sl], md_train[train_sl], atol=1e-6):
            return None

        ti_test = h_test["TVT_input"].to_numpy(dtype=float)[test_sl]
        ti_train = hw_tr["TVT_input"].to_numpy(dtype=float)[train_sl]
        both_ti = np.isfinite(ti_test) & np.isfinite(ti_train)
        n_ti = int(both_ti.sum())
        if n_ti < _DUP_MIN_TVT_INPUT_ROWS:
            return None
        ...
        if n_eval == 0:
            return None
        train_rows = eval_idx + offset
        valid = (train_rows >= 0) & (train_rows < n_train)
        if not valid.any():
            return None

        ref_col, guard_frac = _select_formation_guard_col(hw_tr, train_rows[valid])
        if ref_col is None:
            return None

        reconstructed = tvt_from_contacts(hw_tr, tw_tr, ref_col=ref_col)
        if reconstructed is None or reconstructed.size != n_train:
            return None

        override = np.full(n_eval, np.nan, dtype=float)
        valid_pos = np.where(valid)[0]
        candidate_vals = reconstructed[train_rows[valid]]
        finite_candidates = np.isfinite(candidate_vals)
        override[valid_pos[finite_candidates]] = candidate_vals[finite_candidates]

        eval_row_coverage = float(np.mean(np.isfinite(override)))
        diagnostics: dict[str, object] = {
            "offset": offset,
            "overlap_rows": int(i_hi - i_lo),
            "tvt_input_max_diff": tvt_input_max_diff,
            "n_tvt_input_matched": n_ti,
            "ref_col": ref_col,
            "ref_col_guard_frac": guard_frac,
            "eval_row_coverage": eval_row_coverage,
            "n_eval_rows": n_eval,
        }
        return {"twin_well": wid, "override": override, "diagnostics": diagnostics}
    except Exception:  # never-raise: caller falls back to v1's own result for this well
        return None


# --------------------------------------------------------------------------- #
# ROGII_DUP3: loose-ID direct TVT-copy fallback (new; self-contained, purely
# additive on top of the ROGII_DUP/ROGII_DUP2 blocks above -- see this file's
# module docstring's "ROGII_DUP3" section for the full rationale/leak-safety
# argument). Reuses v1/v2's own _compute_md_offset/eval_mask/load_horizontal
# helpers -- nothing in this block duplicates a constant already defined
# above.
# --------------------------------------------------------------------------- #

_DUP3_MIN_PREFIX_ROWS = 100
_DUP3_PREFIX_RMSE_TOL = 2.0  # ft; ~200x looser than v2's 0.01 ft max-abs-diff gate


def _loose_id_direct_copy(h_test: pd.DataFrame, wid: str, root: Path) -> dict[str, object] | None:
    """Loose ID-match direct-TVT-copy fallback for one test well; never raises.

    Only meant to be called when NEITHER v1's byte-identical
    :func:`find_duplicate_twin` NOR v2's :func:`_contacts_guard_and_reconstruct`
    matched this test well. v2's contacts guard requires known-zone
    ``TVT_input`` to agree to within 0.01 ft (:data:`_DUP_TVT_INPUT_TOL`) --
    appropriate for a byte-identical re-serialization of the same log, but
    too strict for a genuinely RE-LOGGED copy of the same physical well
    (same well ID, fresh acquisition run, small ``GR`` sensor noise/drift and
    a small near-constant ``TVT_input`` re-processing shift). This fallback
    loosens the known-zone agreement check to a prefix RMSE over
    >=100 rows (:data:`_DUP3_MIN_PREFIX_ROWS`) that must be
    < 2.0 ft (:data:`_DUP3_PREFIX_RMSE_TOL`) -- see the module docstring's
    "ROGII_DUP3" section for why this remains a safe same-well confirmation
    (100+ independent known-zone rows agreeing to within 2 ft RMSE by chance
    for two DIFFERENT wells is not physically plausible given this dataset's
    own measured per-well TVT statistics) while still admitting a genuine
    re-log of the SAME well.

    Where the guard passes, this copies the train copy's own ``TVT`` column
    DIRECTLY (not a contacts reconstruction, unlike v2) into the test well's
    evaluation-zone rows, MD-aligned via the same integer row-offset
    construction as v1/v2.

    Guard (all conditions required, never raises -- returns ``None`` on any
    failure or gate miss):
      1. ``wid`` (the test well's own ID) is present among the train wells
         (checked here via the train-split horizontal-well CSV's existence,
         independent of any caller-side pre-filtering).
      2. The test/train MD grids resolve to an integer row offset
         (:func:`_compute_md_offset`) and agree exactly (``atol=1e-6``) over
         their overlap.
      3. Known-zone ``TVT_input`` prefix RMSE over rows both finite in the
         overlap: >=100 rows, RMSE < 2.0 ft.
      4. The train copy has a ``TVT`` column with at least one finite value
         over the train rows that correspond, via the MD offset, to the test
         well's evaluation-zone rows.
    ...
    Returns (on success)::

        {
            "twin_well": wid,
            "override": np.ndarray,  # aligned to eval_mask(h_test) row order;
                                     # finite where the train copy's TVT
                                     # covers that row, NaN elsewhere
            "diagnostics": {...},    # path="loose_id_direct", prefix_rmse,
                                     # n_prefix, eval_row_coverage, ...
        }

    Returns ``None`` if any gate fails or on ANY internal failure (never
    raises -- the caller always falls back to whatever v1/v2's own overlay/
    sub-v8 pipeline already produced for this well).

    Leak safety: see the module docstring's "ROGII_DUP3" leak-safety
    addendum -- only ``MD``/``TVT_input`` (both in the test schema, disjoint
    from the eval zone) are ever read from the test well; the overridden
    value is the matched TRAIN well's own ``TVT`` column, permitted for use
    against any well under this repo's leak rules.
    """
    try:
        train_path = root / "train" / f"{wid}__horizontal_well.csv"
        ...
        ti_train = hw_tr["TVT_input"].to_numpy(dtype=float)[train_sl]
        both_ti = np.isfinite(ti_test) & np.isfinite(ti_train)
        n_prefix = int(both_ti.sum())
        if n_prefix < _DUP3_MIN_PREFIX_ROWS:
            return None
        diffs = ti_test[both_ti] - ti_train[both_ti]
        prefix_rmse = float(np.sqrt(np.mean(diffs**2)))
        if not np.isfinite(prefix_rmse) or prefix_rmse >= _DUP3_PREFIX_RMSE_TOL:
            return None
        ...
        train_tvt = hw_tr["TVT"].to_numpy(dtype=float)
        candidate_vals = train_tvt[train_rows[valid]]
        finite_candidates = np.isfinite(candidate_vals)
        ...
        return {"twin_well": wid, "override": override, "diagnostics": diagnostics}
    except Exception:  # never-raise: caller falls back to v1/v2's own result for this well
        return None
...
def main() -> None:
    ...
    # --- ROGII_DUP2: ID-match contacts-reconstruction fallback (new; see
    # this file's module docstring's "ROGII_DUP2" section). Runs under the
    # SAME `ROGII_DUP` toggle as the block above -- no separate env var --
    # and only ever fills in `dup_results` entries v1's byte-identical
    # search above left unmatched; it never touches an entry already set
    # there (a direct byte-identical copy is exact and always preferred).
    # The existing `merge_duplicate_overrides` call later in this function
    # (unchanged from v1) picks up any new entries automatically since it
    # iterates the same `dup_results` dict. ---
    if dup_enabled:
        t_dup2 = time.time()
        train_well_id_set = set(dup_all_train_wells)
        n_dup2_direct = len(dup_results)
        n_dup2_contacts = 0
        n_dup2_none = 0
        dup2_coverage_weighted_sum = 0.0
        dup2_coverage_row_total = 0
        for well in test_wells:
            if well in dup_results:
                diag = dup_results[well]["diagnostics"]
                print(
                    f"[DUP2] {well}: path=direct twin={dup_results[well]['twin_well']} "
                    f"eval_row_coverage={diag['eval_row_coverage']:.4f}"
                )
                continue
            if well not in train_well_id_set:
                print(f"[DUP2] {well}: path=none (test well ID not found among train wells)")
                n_dup2_none += 1
                continue
            try:
                h_test_dup2 = load_horizontal(well, "test", root)
            except Exception as exc:
                print(f"[DUP2] {well}: path=none (failed to load test well: {exc!r})")
                n_dup2_none += 1
                continue
            contacts = _contacts_guard_and_reconstruct(h_test_dup2, well, root)
            if contacts is None:
                print(f"[DUP2] {well}: path=none (ID match but contacts guard did not pass)")
                n_dup2_none += 1
                continue
            contacts["eval_idx"] = np.where(eval_mask(h_test_dup2))[0]
            dup_results[well] = contacts
            n_dup2_contacts += 1
            diag = contacts["diagnostics"]
            dup2_coverage_weighted_sum += diag["eval_row_coverage"] * diag["n_eval_rows"]
            dup2_coverage_row_total += diag["n_eval_rows"]
            print(
                f"[DUP2] {well}: path=contacts twin={contacts['twin_well']} "
                f"ref_col={diag['ref_col']} offset={diag['offset']} "
                f"overlap_rows={diag['overlap_rows']} "
                f"tvt_input_max_diff={diag['tvt_input_max_diff']:.6f} "
                f"eval_row_coverage={diag['eval_row_coverage']:.4f} "
                f"n_eval_rows={diag['n_eval_rows']}"
            )
        dup2_contacts_coverage = (
            dup2_coverage_weighted_sum / dup2_coverage_row_total if dup2_coverage_row_total else 0.0
        )
        print(
            f"[DUP2] summary: direct={n_dup2_direct} contacts={n_dup2_contacts} "
            f"none={n_dup2_none} (of {len(test_wells)} test well(s)); "
            f"contacts-path eval-row coverage={dup2_contacts_coverage:.4f} "
            f"({time.time() - t_dup2:.1f}s)"
        )

    # --- ROGII_DUP3: loose-ID direct TVT-copy fallback (new; see this file's
    # module docstring's "ROGII_DUP3" section). Runs under the SAME
    # `ROGII_DUP` toggle as DUP/DUP2 above -- no separate env var -- and only
    # ever fills in `dup_results` entries still unmatched after v1's direct
    # byte-match AND v2's strict (0.01 ft) contacts reconstruction; it never
    # touches an entry either of those already produced. The existing
    # `merge_duplicate_overrides` call later in this function (unchanged
    # from v1/v2) picks up any new entries automatically since it iterates
    # the same `dup_results` dict. ---
    if dup_enabled:
        t_dup3 = time.time()
        n_dup3_prior = len(dup_results)
        n_dup3_loose = 0
        n_dup3_none = 0
        dup3_coverage_weighted_sum = 0.0
        dup3_coverage_row_total = 0
        for well in test_wells:
            if well in dup_results:
                continue  # already matched by v1's direct copy or v2's contacts guard
            if well not in train_well_id_set:
                print(f"[DUP3] {well}: path=none (test well ID not found among train wells)")
                n_dup3_none += 1
                continue
            try:
                h_test_dup3 = load_horizontal(well, "test", root)
            except Exception as exc:
                print(f"[DUP3] {well}: path=none (failed to load test well: {exc!r})")
                n_dup3_none += 1
                continue
            loose = _loose_id_direct_copy(h_test_dup3, well, root)
            if loose is None:
                print(f"[DUP3] {well}: path=none (ID match but loose prefix guard did not pass)")
                n_dup3_none += 1
                continue
            loose["eval_idx"] = np.where(eval_mask(h_test_dup3))[0]
            dup_results[well] = loose
            n_dup3_loose += 1
            diag = loose["diagnostics"]
            dup3_coverage_weighted_sum += diag["eval_row_coverage"] * diag["n_eval_rows"]
            dup3_coverage_row_total += diag["n_eval_rows"]
            print(
                f"[DUP3] {well}: path=loose_id_direct twin={loose['twin_well']} "
                f"offset={diag['offset']} overlap_rows={diag['overlap_rows']} "
                f"prefix_rmse={diag['prefix_rmse']:.4f} n_prefix={diag['n_prefix']} "
                f"eval_row_coverage={diag['eval_row_coverage']:.4f} "
                f"n_eval_rows={diag['n_eval_rows']}"
            )
        dup3_loose_coverage = (
            dup3_coverage_weighted_sum / dup3_coverage_row_total if dup3_coverage_row_total else 0.0
        )
        print(
            f"[DUP3] summary: prior(direct+contacts)={n_dup3_prior} loose_id={n_dup3_loose} "
            f"none={n_dup3_none} (of {len(test_wells)} test well(s)); "
            f"loose_id-path eval-row coverage={dup3_loose_coverage:.4f} "
            f"({time.time() - t_dup3:.1f}s)"
        )
